...
$(function() {
  ...
  amplitude.getInstance().init(window._ppgConfig.amplitude, window._ppgConfig.uuid, {
    includeGclid: true,
  ...
  ga('create', window._ppgConfig.ga, 'auto');
  ga('set', 'userId', window._ppgConfig.uuid);
  ga('send', 'pageview');
  (function(h, o, t, j, a, r) {
    h.hj = h.hj || function() {
      (h.hj.q = h.hj.q || []).push(arguments)
    };
    h._hjSettings = { hjid: window._ppgConfig.hotjar, hjsv: 5 };
  ...
  })(document, "script", "serve.albacross.com/track.js", window._ppgConfig.albacross);
  ...
  mixpanel.init(window._ppgConfig.mixpanel);
  ...
  fbq('init', window._ppgConfig.facebook);
  fbq('track', 'PageView');

  var locationMainPage = /^\/[a-z]{2}\/$/i;
  var locationBlogPost = /^\/[a-z]{2}\/blog.*$/i;
  var locationPricingPage = /^\/[a-z]{2}\/pages\/cennik|pricing.*$/;
  var locationWhyUsPage = /^\/[a-z]{2}\/pages\/choose-us|dlaczego-pushpushgo.*$/;

  function visitEvent() {
    var event, properties = { category: 'Marketing' };

    if (locationMainPage.test(location.pathname)) {
      event = 'Visit Home Page';
    }

    if (locationBlogPost.test(location.pathname)) {
      event = 'Visit Blog Post';
      properties.title = window.document.title;
      properties.pathname = location.pathname;
    }

    if (locationPricingPage.test(location.pathname)) {
      event = 'Visit Pricing Page';
    }

    if (locationWhyUsPage.test(location.pathname)) {
      event = 'Viewed Why PushPushGo Page';
    }
