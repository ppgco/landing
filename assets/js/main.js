$(function() {
  ...
  Webinars = (function() {
    Webinars = function() {
      this.webinarsNew = [];
      this.webinarsOld = [];
      this.title = document.getElementById('webinar__title');
      this.chat = document.getElementById('webinar__chat');
      this.video = document.getElementById('webinar__video');
      this.list = document.getElementById('webinar__list');
      this.webinarList = document.getElementById('webinar__options');
      this.webinarDescription = document.getElementById('webinar__description');
      this.shtzu = document.getElementById('shtzu');
      this.initialize();
      return this;
    }

    Webinars.prototype.getUrl = function() {
      var urls = {
        pushpushgo: 'https://api.pushpushgo.com/webinar',
        stppg: 'https://api.stppg.co/webinar',
        localhost: 'https://api.pushpushgo.com/webinar'
      }
    ...
    Webinars.prototype.get = function(onSuccess, onFail) {
      var xmlhttp;
      xmlhttp = new XMLHttpRequest;
      xmlhttp.open('GET', this.getUrl() + '?sort=createdAt');
    ...
    Webinars.prototype.post = function(data, onSuccess, onFail) {
      var xmlhttp;
      xmlhttp = new XMLHttpRequest;
      xmlhttp.open('POST', this.getUrl() + '/lead');
      xmlhttp.setRequestHeader('Content-Type', 'application/json; charset=UTF-8');
      xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
          return onSuccess(xmlhttp.responseText);
    ...
    Webinars.prototype.initialize = function() {
      var self = this;
      if (location.href.indexOf('webinar') === -1) return;

      if (this.shtzu) {
        this.shtzu.addEventListener('submit', function(e) {
          e.preventDefault();
          var data = $(self.shtzu).serializeArray().reduce(function(d, v) {
            d[v.name] = v.value;
            return d
          }, {});
          data.data = new Date().toJSON();

          if (data.webinar === "") delete data.webinar;

          self.post(data, function() {
            var url = location.pathname.replace('-form', '');
            ga('send', 'event', 'user', 'lead');
            self.shtzu.innerHTML = '<p>Dziękujemy za zapis!</p><p>Przypomnimy Ci o webinarium za pomocą wybranych przez Ciebie kanałów.</p><a href="' + url + '">Zobacz nagrania z poprzednich webinariów</a>';
          }, function noop() {
          });
        });
      }

      this.get(function(webinars) {
        try {

          webinars = JSON.parse(webinars) || [];

          webinars.forEach(function(webinar) {
            if (new Date(webinar.createdAt) > new Date()) {
              self.webinarsNew.push(webinar);
            } else {
              self.webinarsOld.push(webinar);
            }
          });

          if (self.webinarsNew.length > 0) {
            self.render(self.webinarsNew[0]);
            self.renderOptionList(self.webinarsNew);
          }
          if (self.webinarsOld.length > 0) {
            self.renderWebinarList(self.webinarsOld);
          }
        } catch (e) {
          console.error(e);
        }
    ...
    Webinars.prototype.render = function(webinar) {
      var videoUrl = function(id) {
        return 'https://www.youtube.com/embed/' + id;
      }

      var chatUrl = function(id) {
        return 'https://www.youtube.com/live_chat?v=' + id + '&embed_domain=' + location.hostname;
      }

      this.video && this.video.setAttribute('src', videoUrl(webinar.youtubeId));
      this.chat && this.chat.setAttribute('src', chatUrl(webinar.youtubeId));

      this.setWebinarDescription(webinar.id);
    }

    Webinars.prototype.renderWebinarList = function(webinars) {
      if (!this.list) return;
      this.list.innerHTML = '';
      for (var index = 0; index < webinars.length; index++) {
        var webinar = webinars[index];
        this.list && this.list.appendChild(this.renderWebinarListItem(webinar));
      }
    }

    Webinars.prototype.renderOptionList = function(webinars) {
      var self = this;
      for (var index = 0; index < webinars.length; index++) {
        var webinar = webinars[index];
        this.webinarList && this.webinarList.appendChild(this.renderWebinarOptionItem(webinar));
      }
      this.webinarList && this.webinarList.addEventListener('change', function(e) {
        self.setWebinarDescription(e.target.value);
      });
    }

    Webinars.prototype.setWebinarDescription = function(webinarId) {
      if (!this.webinarDescription) return;
      for (var i = 0; i < this.webinarsNew.length; i++) {
        var webinar = this.webinarsNew[i];
        if (webinar.id === webinarId) {
          return this.webinarDescription.innerHTML = webinar.description;
        }
      }
    }

    Webinars.prototype.renderWebinarOptionItem = function(webinar) {
      var el = document.createElement('option')
      var date = new Date(webinar.createdAt).toJSON();
      el.innerHTML = webinar.name + date.slice(0, 10) + ' o godzinie ' + date.slice(11, 16);
      el.setAttribute('value', webinar.id);
      return el;
    }

    Webinars.prototype.renderWebinarListItem = function(webinar) {
      var self = this;
      var date = new Date(webinar.createdAt).toJSON();
